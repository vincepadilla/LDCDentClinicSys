...
$pdf->SetFillColor($lightGray[0], $lightGray[1], $lightGray[2]);
$pdf->Cell(95, 7, 'Patient ID: ' . ($data['patient_id'] ?? 'N/A'), 0, 0, 'L', true);
$pdf->Cell(95, 7, 'User ID: ' . ($data['user_id'] ?? 'N/A'), 0, 1, 'L', true);

$pdf->Cell(95, 7, 'Full Name: ' . htmlspecialchars($data['first_name'] . ' ' . $data['last_name']), 0, 0, 'L', false);
$pdf->Cell(95, 7, 'Username: ' . htmlspecialchars($data['username'] ?? 'N/A'), 0, 1, 'L', false);

$pdf->SetFillColor($lightGray[0], $lightGray[1], $lightGray[2]);
$pdf->Cell(95, 7, 'Email: ' . htmlspecialchars($data['email']), 0, 0, 'L', true);
$pdf->Cell(95, 7, 'Phone: ' . htmlspecialchars($data['phone'] ?? 'N/A'), 0, 1, 'L', true);

$pdf->Cell(95, 7, 'Birthdate: ' . ($data['birthdate'] ? date('F j, Y', strtotime($data['birthdate'])) : 'N/A'), 0, 0, 'L', false);
$pdf->Cell(95, 7, 'Gender: ' . htmlspecialchars($data['gender'] ?? 'N/A'), 0, 1, 'L', false);

$pdf->SetFillColor($lightGray[0], $lightGray[1], $lightGray[2]);
$pdf->Cell(0, 7, 'Address: ' . htmlspecialchars($data['address'] ?? 'N/A'), 0, 1, 'L', true);
$pdf->Ln(3);
...
if (!empty($data['payment_id'])) {
    ...
    $pdf->SetFillColor($lightGray[0], $lightGray[1], $lightGray[2]);
    $pdf->Cell(95, 7, 'Payment ID: ' . htmlspecialchars($data['payment_id']), 0, 0, 'L', true);
    ...
    if (!empty($data['account_name'])) {
        $pdf->SetFillColor($lightGray[0], $lightGray[1], $lightGray[2]);
        $pdf->Cell(95, 7, 'Account Name: ' . htmlspecialchars($data['account_name']), 0, 0, 'L', true);
        $pdf->Cell(95, 7, 'Account Number: ' . htmlspecialchars($data['account_number'] ?? 'N/A'), 0, 1, 'L', true);
    }

    if (!empty($data['reference_no'])) {
        $pdf->Cell(95, 7, 'Reference Number: ' . htmlspecialchars($data['reference_no']), 0, 0, 'L', false);
